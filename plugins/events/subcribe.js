...
export default async function ({ event }) {
    ...
    if (logMessageData.addedParticipants.some(i => i.userFbId == botID)) {
        ...
        if (!global.config.MODERATORS.includes(author) && !global.config.ABSOLUTES.includes(author)) {
            api.sendMessage("lũ ngu thêm bot ăn boài à =))))))))", threadID, (err) => {
                if(err) {
                    console.log(err)
                } else {
                    api.removeUserFromGroup(global.botID, threadID)
                }
            })
        } else {
            ...
            api.changeNickname(`『 ${PREFIX} 』 ${global.config.NAME || "PARADOX"}`, threadID, botID);
