...
async function checkAppstate(APPSTATE_PATH, APPSTATE_PROTECTION) {
    ...
    if (!isExists(APPSTATE_PATH, "file")) {
        ...
        logger.custom(getLang('modules.checkAppstate.foundAppstate'), ' PARADOX - LOGIN ');
    }
    ...
    if (APPSTATE_PROTECTION !== true) {
        logger.custom(getLang('modules.checkAppstate.noProtection'), ' PARADOX - LOGIN ');
        ...
        logger.custom(getLang('modules.checkAppstate.error.notSupported'), ' PARADOX - LOGIN ', "\x1b[33m");
        objAppState = await getAppStateNoProtection(APPSTATE_PATH, appState, isReplit, isGlitch);
    }
...
async function getAppStateNoProtection(APPSTATE_PATH, appState, isReplit, isGlitch) {
    ...
    try {
        if (isJSON(appState)) {
            ...
            logger.custom(getLang('modules.checkAppstate.decrypting'), ' PARADOX - LOGIN ');
    ...
    try {
        ...
        if (isReplit) {
            ...
            if (!isJSON(appState)) {
                ...
                objAppState = JSON.parse(appState);
                logger.custom(getLang('modules.checkAppstate.encrypting'), ' PARADOX - LOGIN ');
