    ...
    for (let i = 0; i < pluginCategories.length; i++) {
        ...
        for (const plugin of categoryFiles) {
            ...
            try {
                ...
                console.error(error);
                logger.custom(getLang('modules.loader.plugins.commands.error.failed', { fileName, error }), ' PARADOX - LOADING ', '\x1b[31m');
            }
        }
    }


    logger.custom(getLang('modules.loader.plugins.commands.loaded', { commands: global.plugins.commands.size, total }), ' PARADOX - LOADING ', '\x1b[32m');
...
async function loadCustoms() {
    ...
    for (const plugin of customsFiles) {
        ...
        try {
            ...
            // console.error(error);
            logger.custom(getLang('modules.loader.plugins.customs.error.failed', { fileName, error }), ' PARADOX - LOADING ', '\x1b[31m');
        }
    }

    logger.custom(getLang('modules.loader.plugins.customs.loaded', { customs: global.plugins.customs, total: customsFiles.length }), ' PARADOX - LOADING ', '\x1b[32m');
}
...
async function loadEvents() {
    ...
    for (const plugin of eventsFiles) {
        ...
        try {
            ...
            // console.error(error);
            logger.custom(getLang('modules.loader.plugins.events.error.failed', { fileName, error }), ' PARADOX - LOADING ', '\x1b[31m');
        }
    }

    logger.custom(getLang('modules.loader.plugins.events.loaded', { events: global.plugins.events.size, total: eventsFiles.length }), ' PARADOX - LOADING ', '\x1b[32m');
}
